...
// --- ESTADOS DE ANEXOS (Reactivos desde las props) ---
...
const patientAge = computed(() => {
    const birthDateStr = selectedBudget.value?.patient?.birthDate || selectedPatient.value?.birthDate;
...
onMounted(async () => {
  if (currentId.value) {
    ...
    // Si se pidieron anexos, cargamos los datos necesarios
    if (selectedBudget.value?.patient?.id) {
        const pId = selectedBudget.value.patient.id;
        const promises = [];
        
        if (props.extras.odontogram) {
           promises.push(odontogramStore.fetchOdontogram(pId));
           // Si falta la edad en el presupuesto, traemos al paciente completo
           if (!selectedPatient.value) promises.push(patientsStore.fetchPatientById(pId));
        }
        if (props.extras.evolution) {
...
async function getPDFObject() {
  ...
  // Capturamos TODAS las hojas visibles (ahora incluye anexos porque son visibles)
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // Clonar para asegurar limpieza al renderizar
    const clone = pageElement.cloneNode(true) as HTMLElement;
    
    // Configuración para captura limpia (fixed, fuera de pantalla)
    ...
    // Forzar visualización de elementos internos clonados
...
const printWindow = () => {
    ...
    if (!printRoot) return;
    const printContent = printRoot.innerHTML;
    ...
    newWin.document.write(`
        ...
        <html>
        <head>
            <title>Presupuesto - ${selectedBudget.value.patient.fullName}</title>
            <style>
                ${styles}
                body { margin: 0; font-family: 'Inter', sans-serif; background: white; }
                .a4-page { width: 210mm; min-height: 297mm; margin: 0 auto; page-break-after: always; box-sizing: border-box; }
                .a4-page:last-child { page-break-after: auto; }
                .print\\:break-before-page { page-break-before: always; }
                /* Additional print styles */
                @media print {
                    body { margin: 0; }
                    .a4-page { page-break-after: always; margin: 0; }
                    .a4-page:last-child { page-break-after: auto; }
                }
            </style>
        </head>
    ...
    <div class="w-full max-w-[230mm] flex flex-col items-center gap-6 animate-fade-in">
      ...
      <div class="bg-white p-3 rounded-lg shadow-2xl flex flex-col md:flex-row gap-4 sticky top-4 z-50 px-6 items-center border border-gray-200 print:hidden w-full max-w-4xl justify-between">
        ...
        <div class="flex items-center gap-4 text-sm text-gray-600">
            <span class="font-bold text-gray-400 uppercase tracking-wider text-xs">Mostrando:</span>
            <!-- Indicadores visuales -->
            <span v-if="showPatientFormat" class="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-bold">Paciente</span>
            <span v-if="showDoctorFormat" class="px-2 py-1 bg-gray-100 text-gray-700 rounded-md text-xs font-bold">Interno</span>
            <span v-if="attachOdontogram" class="px-2 py-1 bg-purple-100 text-purple-700 rounded-md text-xs font-bold">Odontograma</span>
            <span v-if="attachEvolution" class="px-2 py-1 bg-green-100 text-green-700 rounded-md text-xs font-bold">Evolución</span>
        </div>
      ...
      <!-- CONTENEDOR DE HOJAS VISIBLES (PARA QUE EL USUARIO LAS VEA) -->
      <div v-else-if="selectedBudget" id="print-root" class="flex flex-col gap-8 w-full items-center print:block print:w-full">
        ...
        <div v-if="showPatientFormat" class="a4-page a4-page-capture bg-white shadow-lg p-[15mm] relative flex flex-col justify-between print:shadow-none print:mb-0 print:break-after-page">
           <!-- ... (Contenido Hoja Paciente igual) ... -->
           <header class="flex justify-between items-start pb-6 border-b-2 border-primary mb-6">
              <div class="flex-1 pr-4">
                <h1 class="text-3xl font-extrabold text-blue-600 uppercase tracking-tight">{{ selectedBudget.tenant.name }}</h1>
        ...
        <!-- HOJA 2: FICHA DOCTOR -->
        <div v-if="showDoctorFormat" class="a4-page a4-page-capture bg-white shadow-lg p-[15mm] relative flex flex-col font-mono text-sm print:break-before-page">
        ...
        <!-- ANEXO 1: ODONTOGRAMA (VISIBLE AHORA) -->
        <!-- Se muestra en pantalla como una hoja más -->
        <div v-if="attachOdontogram" id="annex-odontogram" class="a4-page a4-page-capture bg-white shadow-lg p-[15mm] relative flex flex-col items-center justify-center print:break-before-page">
            ...
            <div class="odontogram-scale-container" style="width: 100%;">
                <Odontogram 
                    v-if="wholeTeeth && Object.keys(wholeTeeth).length > 0"
                    :whole-teeth="wholeTeeth" 
                    :surfaces="surfaces" 
                    :patient-age="Number(patientAge)" 
                    user-role="viewer" 
                />
                <div v-else class="text-center text-gray-400 italic py-10">Cargando gráfico dental...</div>
            </div>
        ...
        <!-- ANEXO 2: EVOLUCIONES (VISIBLE AHORA) -->
