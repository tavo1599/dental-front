    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center gap-3 cursor-pointer" @click="scrollToSection('inicio')">
          ...
          <span class="font-bold text-xl md:text-2xl text-gray-800 tracking-tight leading-none transition-all duration-300"
                :class="{'font-serif tracking-widest uppercase text-lg md:text-xl': theme === 'classic', 'font-light': theme === 'minimal'}">
            {{ tenantData?.name || 'Tu Clínica' }}
          </span>
        </div>

        <!-- 2. MENÚ ESCRITORIO (Centrado/Derecha) -->
        <div class="hidden md:flex items-center space-x-8" :class="{'font-serif uppercase text-xs tracking-wider': theme === 'classic', 'text-sm font-medium': theme === 'minimal'}">
        ...
        <div class="hidden md:block">
          <a 
            ...
            :href="whatsappLink" 
            target="_blank"
            class="text-white px-6 py-2.5 font-bold text-sm transition-all shadow-lg shadow-primary/30 flex items-center gap-2 transform hover:-translate-y-0.5"
            :class="{
              'bg-primary rounded-full hover:bg-opacity-90': theme === 'modern',
              'bg-primary rounded-sm hover:bg-opacity-90': theme === 'classic',
              'bg-primary rounded-xl hover:shadow-xl': theme === 'minimal'
            }"
          >
