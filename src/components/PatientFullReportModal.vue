...
// Tipos
import { PaymentMethod, Gender } from '@/types'; // Importamos Gender para el tipo
...
// --- CORRECCIÓN 1: LOGO (R2 + Cache Buster) ---
const logoSrc = computed(() => {
  const url = authStore.user?.tenant?.logoUrl;
  if (!url) return null;
  
  // Agregamos timestamp para asegurar una carga fresca sin problemas de CORS/Caché
  const timestamp = new Date().getTime();

  // 1. Si es URL absoluta (Cloudflare R2)
...
const formatGender = (gender?: string | Gender) => {
  ...
  switch (gender) {
    ...
    default: return gender; // Si viene algo diferente, lo mostramos tal cual
  }
};
...
<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-start overflow-y-auto pt-10 pb-10 print:static print:bg-white print:p-0 print:h-auto print:overflow-visible print:block">
    
    <div id="printable-report" class="bg-white w-full max-w-4xl rounded-lg shadow-xl p-8 relative print:shadow-none print:w-full print:max-w-none print:p-0 print:m-0">
      ...
      <div v-else-if="selectedPatient" class="space-y-6 text-sm text-gray-800 font-sans print:space-y-4">
        ...
        <header class="border-b-2 border-primary pb-4 flex justify-between items-center print:pt-0 print:pb-2">
          <div>
            <h1 class="text-2xl font-bold text-primary uppercase print:text-lg">{{ authStore.user?.tenant?.name || 'Clínica Dental' }}</h1>
          ...
          <div v-if="logoSrc">
            <!-- Crossorigin + key para forzar recarga -->
            <img :src="logoSrc" class="h-20 object-contain print:h-16" crossorigin="anonymous" />
          </div>
        </header>
        
        <div class="text-center py-2 bg-gray-100 rounded print:bg-transparent print:border-b print:border-gray-300 print:py-1">
        ...
        <section class="print:break-inside-avoid">
          ...
          <div class="grid grid-cols-3 gap-x-4 gap-y-2 print:text-xs print:gap-y-1">
            ...
            <div><span class="font-bold">Historia Nº:</span> {{ selectedPatient.fileCode || selectedPatient.dni }}</div>
            <div><span class="font-bold">Edad:</span> {{ age }} años</div>
            
            <!-- USAMOS LA FUNCIÓN DE FORMATO AQUÍ -->
        ...
        <!-- 3. ANAMNESIS -->
        <section v-if="medicalHistory" class="avoid-page-break">
...
/* --- CORRECCIÓN IMPRESIÓN DE COLORES --- */
* {
